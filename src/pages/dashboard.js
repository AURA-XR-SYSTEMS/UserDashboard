...
export async function initDashboard() {
  ...
  try {
    ...
    if (!user || !user.billing || user.billing.status === "none") {
      chipsEl.innerHTML =
        'No active plan. <a href="plans.html">Choose a plan</a> to unlock downloads.';
    } else {
      const renew = user.billing.renewAt
        ? new Date(user.billing.renewAt).toLocaleDateString()
        : "—";
    ...
    if (user?.credits && typeof user.credits.balance === "number") {
      ...
      const incl = Number.isFinite(user.credits.includedAllowance)
        ? user.credits.includedAllowance
        : 0;
      ...
      const pPct =
        purTotal > 0
          ? Math.max(0, Math.min(100, (purBal / purTotal) * 100))
          : 0;
