...
export async function loadPlans() {
  ...
  const trialExpired = !!(
    user?.billing?.trial?.expiresAt &&
    new Date(user.billing.trial.expiresAt) < new Date()
  );
  ...
  plans.forEach((p) => {
    ...
    const bannerHtml = showTrialOnBasic
          ...
          <div class="row" style="gap:12px; flex-wrap:wrap;">
            ${
              typeof p.credits === "number"
                ? `<span class="chip">${p.credits.toLocaleString()} credits/mo</span>`
                : ""
            }
            ${
              typeof p.workspaces === "number"
                ? `<span class="chip">${p.workspaces} workspaces</span>`
                : ""
            }
            ${
              typeof p.users === "number"
                ? `<span class="chip">${p.users} users</span>`
                : ""
            }
          </div>
